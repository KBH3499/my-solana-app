...
const SmartContractActions = ({ endpoint }) => {
  ...
  useEffect(() => {
    if (connected) {
      ...
      console.log({
        tykeAdminKeyPair: tykeAdminKeyPair?.publicKey?.toString(),
    ...
    while (attempt < retries) {
      ...
      try {
        ...
        if (tx) {
          ...
          console.log(`Attempt ${attempt}: Transaction not found, retrying...`);
      ...
      const args = [
        {
          pairs: [
            {
              lockSeconds: new BN(14), // 1 month
              poolSize: new BN(10_000_000_000),
              userLimit: new BN(3_000_000_000),
              rewardPercentage: new BN(50),
              isOpen: true,
              startTime: new BN(currentTime),
              normalAssetRewardPercentage: new BN(2),
              specialAssetRewardPercentage: new BN(3),
            },
            {
              lockSeconds: new BN(28), // 3 months
            ...
            {
              lockSeconds: new BN(36), // 6 months
            ...
            {
              lockSeconds: new BN(48), // 12 months
  ...
  const stake = async (stakeAmount) => {
    ...
    //   );
      ...
      console.log({
        userWallet,
        userStakingWallet: userStakingWallet.owner.toString(),
        adminTokenAccount,
      });
      ...
      console.log({ time: time?.toString(), bnAmount: bnAmount.toString(), accounts, accounts2 });
  ...
  return (
    <div className="base-container">
      ...
      <button onClick={handleUnStake} disabled={!connected}>
        Unstake
      </button>
      <button onClick={handleUpdate} disabled={!connected}>
        Update Pool
      </button>
      <button
